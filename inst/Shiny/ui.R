...
shinyUI(fluidPage(theme="theme.css",title="WREG",
                  ...
                  navbarPage(img(src="Logo.png", width="80px",height = "40px"),"WREG",
                             tabPanel("Welcome",
                               ...
                               p("Clicking the “Import Data” tab displays three available options: Importing data directly from PeakFQ outputs, importing from the legacy file structure associated with the previous (MatLab) versions of WREG, version 1.05, or importing from the general file format (specified below)."),
                               ...
                               p("To import the data following the general data format, you will need two files in the same directory: SiteInfo.txt and USGSAnnualTimeSeries.txt.  You can then type this directory path in to the “Path to directory” text box under “Select General WREG data”.  The data is then imported from that directory when the “Import data” button under “Import the data” is clicked.  System responses will be printed as text output below this button. Upon successful import, the imported sites will be listed in this text."),
                               p("The general format for the SiteInfo.txt should contain the following columns (with headers in parentheses): the station identification number (stationID), the latitude and longitude of the stations (latitude and longitude), the regional skew computed for each site (regionalSkew), the at-site skew value (skew), the standard deviation of the Log-Pearson Type-III distribution used to fit the series (standardDeviation), a series of streamflow characteristics to be evaluated (Q#, where # indicates a specific return period), Q#.k (the characteristic-specific kappa value from the fitted Log-Pearson Type-III distribution, where again # specifies a certain return period), and any explanatory variables to be used for analysis.  The annual time seires file (USGSAnnualTimeSeries.txt) is identical to the one used in WREG version 1.05 (MatLab)."),
                             ...
                             navbarMenu("Import Data",
                                        tabPanel("Import PEAK-FQ data",
                                                 ...
                                                 hr(),
                                                 h3("Select the basin information file"),
                                                 ...
                                                 fileInput('gisFile',
                                                           'Basin information file'),
                                                 hr(),
                                                 h3("Select Peak FQ data"),
                                                 h6("Input the path to a directory that contains all PeakFQ files for each site in the site information file. Files can be contained in the main path or in subdirectories of the specified path. Each site should be represented by one and only one .EXP and .PRT file."),
                                                 textInput('pfqPath',
                                                           'Path to directory'),
                                                 hr(),
                                                 h3("Import the data"),
